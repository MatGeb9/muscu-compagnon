...
document.addEventListener("click", e => {
  ...
  if (a === "del-set") { const ex = state.live.exercises[ei]; const s = ex.sets[si]; const lbl = s.isWarmup ? "l'échauffement" : s.drop ? "la série dégressive" : `la série ${s.setIndex}`; if (!confirm(`Supprimer ${lbl} de ${ex.name} ?`)) return; ex.sets.splice(si, 1); let n = 0; ex.sets.forEach(s => { if (!s.isWarmup && !s.drop) s.setIndex = ++n; }); return render(); } // supprime UNE série pendant la séance (ex. 5→4) + renumérote les séries de travail (confirmation anti-clic accidentel)
